...
OF_IMAGE_HEIGHT, OF_IMAGE_WIDTH = 224,224
# ----------------------------
...
def compute_optical_flow_sequence_lk(frames, target_size=(OF_IMAGE_WIDTH, OF_IMAGE_HEIGHT), tau=10):
    """
...
def run_inference_on_video(video_path, model, num_frames=30):
    ...
    flow_sequence = compute_optical_flow_sequence_lk(frames, target_size=(OF_IMAGE_HEIGHT, OF_IMAGE_WIDTH), tau=10)
...
# ----------------------------
# Function to Display the Optical Flow Video Conversion
# ----------------------------
def display_optical_flow_video(video_path, num_frames=30, delay=100):
    """
    Reads the recorded video, computes the optical flow sequence using the Lucas-Kanade method,
    and displays the optical flow frames as a video.
    
    The optical flow image is processed using the COLORMAP_BONE colormap.
    
    Press 'q' to exit the optical flow window.
    """
    frames = sample_frames_from_video(video_path, num_frames)
    if len(frames) < 2:
        ...
        return
    frames = ensure_video_resolution(frames, target_size=(320, 240))
    flow_sequence = compute_optical_flow_sequence_lk(frames, target_size=(OF_IMAGE_WIDTH, OF_IMAGE_HEIGHT), tau=10)
    
    for flow in flow_sequence:
        flow_img = (flow[:, :, 0] * 255).astype(np.uint8)
        color_flow = cv2.applyColorMap(flow_img, cv2.COLORMAP_BONE)
        display_size = (640, 480)
        color_flow_resized = cv2.resize(color_flow, display_size)
        cv2.imshow("Optical Flow Video", color_flow_resized)
        if cv2.waitKey(delay) & 0xFF == ord('q'):
            break
    cv2.destroyWindow("Optical Flow Video")
...
def process_recorded_video(video_path):
    ...
    Then displays the optical flow video.
    """
    ...
    time.sleep(2)
    display_optical_flow_video(video_path)
...
model_path = r"D:\MAPUA\CNN_GRU_VarietyofOF\saved_models\CNNGRU_model_Date_Time2025_02_24__02_06_20_Loss_0.15555697679519653__Accuracy_0.9482758641242981.h5"
try:
...
# ----------------------------
# For this example, we set the window size to 1024x600.
WINDOW_WIDTH = 1024
WINDOW_HEIGHT = 600
root = tk.Tk()
...
content_frame.pack(expand=True, fill="both", padx=CONTENT_PADX, pady=CONTENT_PADY)

# ----------------------------
# Live Camera Feed Frame
# ----------------------------
feed_frame = tk.Frame(content_frame, bg=CONTENT_BG)
feed_frame.pack(expand=True, fill="both", padx=CONTENT_PADX, pady=(CONTENT_PADY, 0))
video_label = tk.Label(feed_frame, bg=CONTENT_BG)
video_label.pack(expand=True, fill="both")

# Info Frame (Status and Timer) – Retained as before
...
status_label = tk.Label(info_frame, text="Status: Idle", bg=INFO_BG, fg=TEXT_COLOR,
                        font=FONT_STATUS, wraplength=WINDOW_WIDTH-100, justify="center")
...
# Button Frame (Buttons placed below the live feed)
